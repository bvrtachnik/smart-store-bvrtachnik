...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
    """
    Replace outlier discount values (>1.0) in the 'CurrentDiscountPercent' column
    for Clothing category with the median discount of other valid Clothing items.
    """
    logger.info(f"FUNCTION START: remove_outliers with dataframe shape={df.shape}")
    initial_count = len(df)

    # Condition: Clothing category + discount > 1.0
    condition = (df['Category'] == 'Clothing') & (df['CurrentDiscountPercent'] > 1.0)

    # Median of valid Clothing discount values (<= 1.0)
    valid_discounts = df.loc[(df['Category'] == 'Clothing') & (df['CurrentDiscountPercent'] <= 1.0), 'CurrentDiscountPercent']
    clothing_median = valid_discounts.median()

    # Logging before replacing
    correction_count = df[condition].shape[0]
    logger.info(f"Replacing {correction_count} Clothing discount outliers with median: {clothing_median}")

    # Replace outliers with median
    df.loc[condition, 'CurrentDiscountPercent'] = clothing_median

    logger.info(f"{len(df)} records remaining after correcting outliers.")
    return df
